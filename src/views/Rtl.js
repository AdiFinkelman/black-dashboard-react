...
function Dashboard(props) {
  ...
  useEffect(() => {
    const fetchHistoricalData = async () => {
      ...
      try {
        ...
        console.error("שגיאה בקליטת המידע:", error);
        setError("מטבע זה לא קיים");
      }
      setIsLoading(false);
    };
    fetchHistoricalData();
  ...
  const aggregateDataByWeek = () => {
    ...
    const aggregatedData = historicalPrices.reduce((acc, data) => {
      const date = new Date(data.date);
      const week = `שבוע ${getWeekNumber(date)}`;
      if (!acc[week]) {
      ...
      <div className="content">
        <Row className="align-items-center mb-4">
          <Col md="4" className="d-flex">
            <form onSubmit={handleSearch} className="d-flex w-100">
              <Input type="text" name="symbol" placeholder="הכנס מטבע (לדוגמה: BTC)" />
              <Button type="submit" className="ml-2">חיפוש</Button>
            </form>
          </Col>
        ...
        {isLoading ? (
          ...
          <>
            <Card>
              <CardHeader>
                <CardTitle tag="h4">גרף בזמן אמת - {coinName}</CardTitle>
                {currentPrice !== null && <h5>מחיר נוכחי: ${currentPrice}</h5>}
              </CardHeader>
